...
local function value_wrap(value)
    return function() feedkeys(value, 'n') end
end
...
local function make_find_till_func(key, is_flash, just_key)
    return function()
        ...
        feedkeys(key .. (just_key and '' or vim.g.last_motion_char), mode .. 't')
    end
end
...
local function repeat_wrap(prev_func, next_func, reversed)
    ...
    return function(...)
        ...
        if vim.tbl_contains({ 'v', 'V', 'CTRL-V', 'n' }, vim.fn.mode()) then
            if is_builtin then
                vim.b.last_prev_function = nil
                vim.b.last_next_function = nil
            elseif is_flash_find_till then
