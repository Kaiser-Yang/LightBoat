...
--- @class LFUCache.KVPair
--- A key-value pair in the LFU cache.
--- @field key any The key associated with this pair.
--- @field value any The value associated with this key.
...
function M:full() return self.size == self.capacity end

function M:last_vis(key)
  assert(M:contains(key))
  return self.gmt_last_vis[key]
end

--- Check if the cache contains a key.
--- @param key any The key to check in the cache.
--- @return boolean True if the key is in the cache, false otherwise.
function M:contains(key) return self.kv_map[key] ~= nil end
...
--- If the new capacity is less than the current size,
--- items will be evicted until the size is less than or equal to the new capacity.
--- @param capacity number The new capacity of the cache.
--- @return LFUCache.KVPair[] A list of key-value pairs that were evicted from the cache.
function M.set_capacity(self, capacity)
  assert(capacity > 0, 'Capacity must be greater than 0')
  self.capacity = capacity
  local res = {}
...
function M:_evict()
  ...
  assert(M:full() and node, 'You can only evict when cache is full')
  remove_node(list, node)
  local key = node.key
